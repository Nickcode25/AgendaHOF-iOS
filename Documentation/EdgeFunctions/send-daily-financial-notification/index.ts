...
const APNS_TOPIC = 'com.agendahof.app' // Bundle ID
...
serve(async (req) => {
  try {
    ...
    // Get all owners with active device tokens
    const { data: deviceTokens, error: tokensError } = await supabase
      ...
      .select(`
        ...
        environment,
        user_id,
        user_profiles!inner(id, is_owner)
      `)
      .eq('is_active', true)
      .eq('user_profiles.is_owner', true)
    ...
    console.log(`✅ Found ${deviceTokens?.length || 0} active devices for owners`)
